 ...
 * Hoja de Trabajo 8
 * 27-03-2023
 * Clase VectorHeap tomada del libro de Duane A. Bailey, Java Structures (está levemente modificada)
 */
...
public class VectorHeap <T> implements PriorityQueue<T>{
    /**
     ...
     * 
     * @param proceso The value to be added.
     */
    public void add(T t)
    {
        data.add(t);
    ...
    protected void percolateUp(int leaf)
    {
        ...
        while (leaf > 0 &&
          (((Comparable) value).compareTo((data.get(parent))) < 0))
        {
        ...
        while (root < heapSize) {
            ...
            if (childpos < heapSize)
            {
                if ((right(root) < heapSize) &&
                  ((((Comparable) data.get(childpos+1))).compareTo((data.get(childpos))) < 0))
                {
                    childpos++;
                }
